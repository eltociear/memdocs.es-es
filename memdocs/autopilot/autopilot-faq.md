...
| ¿Cómo autoriza un cliente a un OEM o un asociado de canal para que registre dispositivos AutoPilot en el nombre del cliente? | Antes de que un OEM o un asociado de canal pueda registrar un dispositivo para el piloto automático para un cliente, el cliente debe dar su consentimiento en primer lugar. El proceso de consentimiento comienza con el OEM o el asociado de canal enviando un vínculo al cliente que dirige al cliente a una página de consentimiento en MSfB. Para obtener más información, consulte [registro](registration-auth.md). |
| ¿Hay alguna restricción si un cliente empresarial tiene dispositivos registrados en MSfB y después quiere que los dispositivos se administren mediante un proveedor de soluciones en la nube (CSP) mediante el centro de Partners? | El cliente empresarial debe eliminar los dispositivos de MSfB antes de que el CSP pueda cargarlos y administrarlos en el centro de Partners. | 
...
| ¿El OEM necesita administrar o recopilar archivos de imágenes personalizados de los clientes? ¿Necesitan cargar imágenes en Microsoft? | Sin cambios, los OEM solo envían el CBRs como de costumbre a Microsoft. No se envía ninguna imagen a Microsoft para habilitar Windows AutoPilot. Windows AutoPilot solo Personaliza OOBE y permite configuraciones de directivas (deshabilita la cuenta de administrador, por ejemplo). |
...
| ¿Qué nueva información se debe enviar del OEM a Microsoft? | Nada, a menos que el OEM opte por registrar el dispositivo en nombre del cliente. En este caso, deben cargar el archivo CSV de ID. de dispositivo en el centro de Partners de Microsoft. O bien, pueden usar la API directa de OEM. |
...
| Pregunta | Respuesta |
| --- | --- |
| Cada hash de hardware que envíe el OEM debe contener los datos siguientes:<br> -UUID de SMBIOS (identificador único universal)<br> -Dirección MAC (Media Access Control)<br> -Número de serie de disco único (si usa la herramienta de activación 3,0 de Windows 10 OEM)? | Sí. Puesto que Windows AutoPilot se basa en la capacidad de identificar de forma exclusiva los dispositivos que se aplican a la configuración de la nube, es fundamental enviar hashes de hardware que cumplan los requisitos descritos. |
| ¿Cuál es el motivo por el que necesita el UUID de SMBIOS, la dirección MAC y el número de serie de disco en los detalles del hash de hardware? | Para crear el hash de hardware, estos campos son necesarios para identificar un dispositivo, ya que las partes del dispositivo se agregan o se quitan. Como no tenemos un identificador único para dispositivos Windows, estos campos son la mejor lógica para identificar un dispositivo. |
...
| [Autopilot para dispositivos existentes](existing-devices.md) | Ofrece una ruta de actualización a Windows AutoPilot para todos los dispositivos de Windows 7 y Windows 8 existentes. |
...
| ¿Es posible que los dispositivos que se han rescrito en imágenes varias veces pasen por el piloto automático? ¿Qué significa el mensaje de error "este usuario no está autorizado a inscribirse"? Código de error 801c0003. |Existen límites en cuanto al número de dispositivos que puede inscribir un usuario Azure AD determinado en Azure AD y el número de dispositivos que se admiten por usuario en Intune. Estos límites son configurables, pero no infinitos. Encontrará este error con frecuencia si vuelve a usar los dispositivos, o incluso si revierte a las instantáneas de máquina virtual anteriores.|
| ¿Qué ocurre si un dispositivo está registrado en un agente malintencionado? | Por diseño, Windows AutoPilot no aplica un perfil hasta que el usuario inicia sesión con el inquilino correspondiente para el perfil configurado mediante el proceso de inicio de sesión Azure AD. Por ejemplo, si badguys.com registra un dispositivo propiedad de contoso.com, en el peor de los usuarios se le dirigirá al usuario para que inicie sesión en badguys.com. Cuando el usuario escribe su correo electrónico y contraseña, la información de inicio de sesión se redirige a través de Azure AD a la autenticación de Azure AD adecuada y se solicita al usuario que inicie sesión en contoso.com. Dado que contoso.com no coincide con badguys.com como el inquilino, el perfil de Windows AutoPilot no se aplicará y se producirá el Azure AD OOBE normal.|
| ¿Dónde se almacenan los datos de Windows AutoPilot? |Los datos del programa piloto de Windows se almacenan en el Estados Unidos (EE. UU.), no en una nube soberano, incluso cuando el inquilino de Azure AD está registrado en una nube soberano. Este almacenamiento se aplica a todos los datos de Windows AutoPilot, independientemente del portal que se use para implementar AutoPilot.|
| ¿Por qué los datos de Windows AutoPilot se almacenan en Estados Unidos y no en una nube soberano?| No se almacenan los datos del cliente, solo los datos empresariales que permiten a Microsoft proporcionar un servicio. Esta es la razón por la que es adecuado almacenar los datos en Estados Unidos. Los clientes pueden dejar de suscribirse al servicio en cualquier momento. En ese caso, Microsoft quita los datos empresariales. AutoPilot no se admite actualmente en ninguna nube soberano. |
